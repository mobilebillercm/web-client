...
$channel->exchange_declare(
    parse_ini_file("global-var-config.ini", true)['EXCHANGES']['USER_PASSWORD_CHANGE_SSL_EXCHANGE'],
    'fanout',
    false,
    true,
    false);
...
list($queue_name, ,) = $channel->queue_declare(
    parse_ini_file("global-var-config.ini", true)['QUEUES']['WEB_CLIENT_USER_PASSWORD_CHANGED_SSL_QUEUE'],
    false,
    true,
    false,
    false);
...
echo " [*] Waiting for password changed \n\n To exit press CTRL+C\n";
...
$callback = function ($msg) {
    ...
    try {
        ...
        $url = parse_ini_file("global-var-config.ini", true)['URLS']['HOST_WEB_CLIENT'] . '/api/change-user-password';
