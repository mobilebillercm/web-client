...
use PhpAmqpLib\Connection\AMQPStreamConnection;
    ...
    $channel->exchange_declare(
        parse_ini_file("global-var-config.ini", true)['EXCHANGES']['ACTIVITY_CONTRACTORS_RECEIVED_EXCHANGE'],
        'fanout',
        false,
        true,
        false);
    ...
    list($queue_name, ,) = $channel->queue_declare(
        parse_ini_file("global-var-config.ini", true)['QUEUES']['NOTIFICATION_ACTIVITY_CONTRACTORS_RECEIVED_QUEUE'],
        false,
        true,
        false,
        false);
    ...
    try {
        ...
        $tokenUrl = parse_ini_file("global-var-config.ini", true)['URLS']['HOST_NOTIFICATION'] . '/oauth/token';


        $tokenData = $client->post($tokenUrl, [
            'form_params' => [
                'grant_type' => 'client_credentials',
                'client_id' => parse_ini_file("global-var-config.ini", true)['LOGINS']['NOTIFICATION_CLIENT_ID'],
                'client_secret' => parse_ini_file("global-var-config.ini", true)['LOGINS']['NOTIFICATION_CLIENT_SECRET'],
            ],
        ]);
    ...
    try {
        ...
        $url = parse_ini_file("global-var-config.ini", true)['URLS']['HOST_NOTIFICATION'] . '/api/message/receivedapplication';
